...
class App {
  ...
  private loadItems<T>(
    ...
    extraSetup?: (itemData: any, item: T) => void,
  ): void {
  ...
  private setupUserBooks(userData: any, user: User): void {
    user.borrowedBooks = userData.borrowedBooks.map(
      (bookData: any) =>
        new Book(
          bookData.title,
          bookData.author,
  ...
  private renderLists(): void {
    this.renderList(
      "book-list-items",
      this.bookLibrary.getItems(),
      this.createBookListItem.bind(this),
    );
    this.renderList(
      "user-list-items",
      this.userLibrary.getItems(),
      this.createUserListItem.bind(this),
    );
  }
  ...
  private setupEventListeners(): void {
    ...
    document
      .getElementById("book-list-items")
      ?.addEventListener("click", (event) => this.handleBookActions(event));
    document
      .getElementById("user-list-items")
      ?.addEventListener("click", (event) => this.handleUserActions(event));

    const borrowButton = document.getElementById("saveButton");
    if (borrowButton) {
      borrowButton.addEventListener("click", () => {
        const bookIndex = parseInt(
          borrowButton.getAttribute("data-index") || "0",
        );
  ...
  private searchBooks(): void {
    const searchTerm = (
      document.getElementById("book-to-search") as HTMLInputElement
    ).value;
    ...
    if (foundBooks.length > 0) {
      foundBooks.forEach((book, index) =>
        list.appendChild(this.createBookListItem(book, index)),
      );
    } else {
  ...
  private handleBookActions(event: Event): void {
    ...
    if (target.classList.contains("borrow-btn")) {
      ...
      userId
        ? this.returnBook(bookIndex, parseInt(userId))
        : this.setupBorrow(bookIndex);
  ...
  private handleUserActions(event: Event): void {
    ...
    if (user && user.borrowedBooks.length > 0) {
      ...
      this.modalManager.setModalContent(
        "exampleModalToggle2",
        `Щоб видалити користувача, поверніть: ${borrowedBooks}`,
      );
  ...
  private addBook(): void {
    ...
    const author = (
      document.querySelector('input[placeholder="Автор"]') as HTMLInputElement
    ).value;
    const year = (
      document.querySelector(
  ...
  private addUser(): void {
    const name = (
      document.querySelector('input[placeholder="Ім\'я"]') as HTMLInputElement
    ).value;
    const email = (
    ...
    this.modalManager.setModalContent(
      "exampleModalToggle2",
      `Користувача ${user.name} з ID: ${userId} додано!`,
    );
    this.modalManager.open("exampleModalToggle2");
    this.saveAndRenderUsers();
  }
  ...
  private checkUserAndBorrow(bookIndex: number, userId: number): void {
    ...
    if (user.borrowedBooks.length >= 3) {
      this.modalManager.setModalContent(
        "exampleModalToggle2",
        "Користувач вже має 3 позичені книги!",
      );
      this.modalManager.open("exampleModalToggle2");
      return;
    }
    ...
    this.modalManager.setModalContent(
      "exampleModalToggle2",
      `Книгу ${book.title} позичено користувачу ${user.name}!`,
    );
    this.modalManager.open("exampleModalToggle2");
  }
  ...
  private returnBook(bookIndex: number, userId: number): void {
    ...
    if (book.borrowedBy === userId) {
      ...
      this.modalManager.setModalContent(
        "exampleModalToggle2",
        `Книгу ${book.title} повернено користувачу ${user.name}!`,
      );
    } else {
      this.modalManager.setModalContent(
        "exampleModalToggle2",
        "Ця книга не була позичена цим користувачем!",
      );
    }
